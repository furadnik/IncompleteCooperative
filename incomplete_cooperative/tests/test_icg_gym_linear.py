...
class TestICGGymLinear(TestCase, GymMixin):
    def test_init(self):
        ...
        self.assertEqual(icg_gym.incomplete_game.number_of_players, icg_gym_wrapper.number_of_players)

    def test_action_masks(self):
        icg_gym = self.get_gym()
        icg_gym_wrapper = ICG_Gym_Linear(icg_gym)
        self.assertEqual(icg_gym_wrapper.action_masks().shape, (icg_gym_wrapper.number_of_players,))
    ...
    def test_reset(self):
        ...
        self.assertEqual(state.shape, (icg_gym_wrapper.number_of_players,))
    ...
    def test_step(self):
        icg_gym = self.get_gym()
        icg_gym_wrapper = ICG_Gym_Linear(icg_gym)
        num_pairs = icg_gym_wrapper.number_of_players * (icg_gym_wrapper.number_of_players - 1) // 2  # There are n choose 2 coalitions of size two
