...
class RootLinearApproximation:
    ...
    def __init__(self, game: IncompleteCooperativeGame, epsilon: float):
        ...
        # Remove duplicates
        self.queried_coalitions = list(set(self.queried_coalitions))
...
class MXApproximation:
    def __init__(
        self, game: IncompleteCooperativeGame, alpha: float, beta: float, eps: float
    ):
        self.number_of_players = game.number_of_players
        self.values = game._values[:, 1]
        ...
        self.multiplicative_factor = self.compute_multiplicative_factor()
        # self.singleton_values = get_singletons(self.number_of_players) #returns vector of values of singletons of length number_of_players
    ...
    def construct_approximation(self):
        ...
        for k in range(len(k_values)):
            for r in range(len(r_values)):
                ...
                while self.values[coalition.id] >= k_values[k] * r_values[r] / (
                    2 * self.alpha
                ):
                    ...
                    for player in coalition.players:
                        if not additive_vector[player] >= r_values[r] / (
    ...
    def approx_xos_subroutine(self, coalition: Coalition) -> np.ndarray:
        ...
        for player in coalition.players:
            ...
            # print(f"Subcoalition: {subcoalition.id}")
            # print(f"Extended subcoalition: {extended_subcoalition.id}")
            # input()
            self.queried_coalitions.append(
                extended_subcoalition
            )  # Here, values are queried

            additive_vector[player] = (
                self.values[extended_subcoalition.id] - self.values[subcoalition.id]
            )
