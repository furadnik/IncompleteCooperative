...
from .protocols import (Game, GameGenerator, GapFunction, IncompleteGame,
...
def _get_act_sequence_exploitability(game: MutableIncompleteGame, full_game: Game, action_sequence: ActionSequence,
                                     known_coalitions: list[Coalition], gap_func: GapFunction
    ...
    game.compute_bounds()
    return action_sequence, gap_func(game)


def get_exploitabilities_of_action_sequences(
        game: MutableIncompleteGame, full_game: Game, gap_func: GapFunction,
        max_size: int | None = None, processes: int = 1
    ...
    with Pool(processes=processes) as p:
        return p.starmap(
            ...
            ((game, full_game, seq, known_coalitions, gap_func)
             for seq in possible_action_sequences(game, max_size=max_size)))


def sample_exploitabilities_of_action_sequences(
        game: MutableIncompleteGame, full_game_generator: GameGenerator, gap_func: GapFunction, samples: int = 1,
        **kwargs: Any
) -> tuple[list[ActionSequence], np.ndarray[Any, np.dtype[Value]]]:
    """Sample exploitabilities of action sequences."""
    LOGGER.info(f"{int(time.time())} Sampling exploitabilities for {game}")
    ...
    initial_run = list(get_exploitabilities_of_action_sequences(
        game, full_game, gap_func, **kwargs))
    ...
    for i in range(1, samples):
        start = int(time.time())
        LOGGER.info(f"{start}, Sampling exploitabilities for {game}, sample {i + 1}")
        full_game = full_game_generator(game.number_of_players)
        game.set_known_values(full_game.get_values(initially_known_coalitions),
                              initially_known_coalitions)
        values[i] = np.fromiter((x[1] for x in get_exploitabilities_of_action_sequences(game, full_game, gap_func,
                                                                                        **kwargs)),
                                Value, initial_values.shape[0])
        LOGGER.info(f"Took {int(time.time()) - start} seconds ({(int(time.time()) - start) / 60} minutes).")
    return actions, values


def get_exploitabilities_of_action_sequence(
        game: MutableIncompleteGame, full_games: Iterable[Game],
        action_sequence: ActionSequence, gap_func: GapFunction, processes: int = 1
) -> Iterable[Value]:
    ...
    with Pool(processes=processes) as p:
        return map(lambda x: x[1], p.starmap(
            _get_act_sequence_exploitability,
            ((game, full_game, action_sequence, known_coalitions, gap_func)
...
def get_stacked_exploitabilities_of_action_sequences(
        ...
        action_sequences: Iterable[ActionSequence], gap_func: GapFunction, processes: int = 1
) -> Iterable[np.ndarray]:
    """Get the expected exploitabilities of a list of action sequences."""
    number_of_games = len(full_games)
    for action_sequence in action_sequences:
        yield np.fromiter(
            get_exploitabilities_of_action_sequence(game, full_games, action_sequence, gap_func, processes),
